...
bool LevelManagerClass::Initialize()
{
	...
	particleOne->rate = 1000;
	particleOne->speed = { 100.0f, 50.0f };
	particleOne->lifetime = 600;
	particleOne->isHeapEmit = true;
	particleOne->emitDirection = EmitDirection::Front;
	particleOne->heapEmitInterval = 1000;
	particleOne->heapEmitRepeats = -1;
	particleOne->color = point3d(1, 0, 0);
	particleOne->spread = { PI, PI };
	particleOne->opacity = { 1, 0 };

	Entity* particleOneEntity2 = entityStorage->CreateEntity("particleOneEntity2");
	transform = particleOneEntity2->AddComponent<Transform>();
	ParticleEmitter* particleOne2 = particleOneEntity2->AddComponent<ParticleEmitter>();
	transform->position = point3d(100.0f, 100.0f, 100.0f);
	particleOne2->rate = 2000;
	particleOne2->speed = { 100.0f, 50.0f };
	particleOne2->lifetime = 600;
	particleOne2->isHeapEmit = true;
	particleOne2->emitDirection = EmitDirection::Front;
	particleOne2->heapEmitInterval = 1000;
	particleOne2->heapEmitRepeats = -1;
	particleOne2->color = point3d(1, 1, 0);
	particleOne2->spread = { PI, PI };
	particleOne2->opacity = { 1, 0 };
	...
	particleTwo->rate = 800;
	particleTwo->speed = { 15.0f, 0.0f };
	//particleOne->lifetime = 100;
	particleTwo->isHeapEmit = true;
	particleTwo->heapEmitInterval = 1000;
	particleTwo->heapEmitRepeats = -1;
	particleTwo->emitDirection = EmitDirection::Up;
	particleTwo->color = point3d(0, 1, 0);
	particleTwo->spread = { 2000.0f, -1.0f };

	Entity* particleThreeEntity = entityStorage->CreateEntity("ParticleThreeEntity");
	transform = particleThreeEntity->AddComponent<Transform>();
	ParticleEmitter* particleThree = particleThreeEntity->AddComponent<ParticleEmitter>();
	transform->position = point3d(200.0f, 100.0f, 100.0f);
	particleThree->rate = 200;
	particleThree->lifetime = 1000;
	particleThree->speed = { 1.0f, 20.0f };
	particleThree->emitDirection = EmitDirection::Front;
	particleThree->opacity = { 1.0f, 0 };
	particleThree->color = point3d(1, 1, 1);
	float x = PI;
	particleThree->spread = { PI / 6, PI / 6 };
	//particleThree->isReverse = true;
	particleThree->size = { 1.0f, 3.0f };

	Entity* particleFourEntity = entityStorage->CreateEntity("particleFourEntity");
	transform = particleFourEntity->AddComponent<Transform>();
	ParticleEmitter* particleFour = particleFourEntity->AddComponent<ParticleEmitter>();
	transform->position = point3d(200.0f, 200.0f, 100.0f);
	particleFour->rate = 4000;
	particleFour->lifetime = 400;
	particleFour->speed = { 1.0f, 200.0f };
	particleFour->emitDirection = EmitDirection::Front;
	particleFour->opacity = { 1, 0 };
	particleFour->color = point3d(0.5f, 0, 1);
	particleFour->isReverse = true;
	particleFour->spread = { PI / 2, PI / 2 };
	//particleThree->isReverse = true;
	particleFour->size = { 1.0f, 3.0f };
	/////////////////////////
